...
export function HeleLinjen ({totalEmission}) { // denne funksjonen returnerer et array med så mange LinjeElement tidslinjen skal være
    ...
    for (var i = 0; i < totalEmission; i+= 0.1){
      colorVariableNumber = Math.floor((i/totalEmission)*10) // fordelingen av grønn, oransje, rød på skalaen 0-10.
      if (colorVariableNumber < 4) { elementArray.push(<LinjeElement elementfargen = "#35C16D"/>)}
      if (colorVariableNumber >= 4 && colorVariableNumber < 8) {elementArray.push(<LinjeElement elementfargen = "#FFA42D"/>)}
      if (colorVariableNumber >= 8) {elementArray.push(<LinjeElement elementfargen = "#D73704"/>)}
